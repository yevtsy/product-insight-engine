...
import com.google.ai.insight.aggregator.tools.InstagramScoutTool;
...
/**
 * LLM Agent for Instagram comment analysis using Google ADK.
 * This agent uses Gemini 2.5 Flash model to fetch and analyze Instagram comments.
 */
@Slf4j
@Component
@RequiredArgsConstructor
public class InstagramScoutLlmAgent {

    private final InstagramScoutTool instagramScoutTool;
    private BaseAgent agent;
    private InMemoryRunner runner;

    @PostConstruct
    public void init() {
        log.info("Initializing Instagram Scout LLM Agent");

        this.agent = LlmAgent.builder()
                .name("instagram-scout")
                .description("Agent for fetching and analyzing comments from Instagram media posts")
                .instruction("""
                        You are an Instagram comment analysis agent. Your role is to:
                        1. Fetch comments from Instagram media posts using the fetchComments tool
                ...
                .tools(FunctionTool.create(instagramScoutTool, "fetchComments"))
                .build();

        this.runner = new InMemoryRunner(agent);

        log.info("Instagram Scout LLM Agent initialized successfully");
    }
    ...
    /**
     * Analyzes feedback for a feature by fetching and processing Instagram comments.
     *
    ...
    public String analyzeFeedback(String featureName, java.util.List<String> keywords, int maxComments) {
        log.info("Analyzing Instagram feedback for feature: {} with keywords: {}", featureName, keywords);
        ...
        try {
            ...
            Session session = runner.sessionService()
                    .createSession("instagram-scout-app", "aggregator")
                    .blockingGet();
            ...
            log.error("Error analyzing Instagram feedback", e);
